...
import { CadastroClienteFormDataInterface, CadastroDiaristaFormDataInterface } from 'data/@types/FormInterface';
...
export default function useAlterarDados(){
    ...
    function getResolver(){
        let resolver = FormSchemaService.userData().concat(FormSchemaService.contact());
        ...
        async function updateUser(
            ...
            ApiServiceHateoas(
                ...
                async (request) => {
                    ...
                    try {
                        const nascimento = TextFormatService.dateToString(data.usuario.nascimento as Date),
                        cpf = TextFormatService.getNumbersFromText(data.usuario.cpf),
                        telefone = TextFormatService.getNumbersFromText(data.usuario.telefone),
                        userData = {
                            ...data.usuario,
                            nascimento,
                            cpf,
                        ...
                        if(!userData.password || !userData.password_confirmation || !userData.new_password){
                        ...
                        const updatedUser = (await request<UserInterface>({
                            data: userData,
                        })).data;
