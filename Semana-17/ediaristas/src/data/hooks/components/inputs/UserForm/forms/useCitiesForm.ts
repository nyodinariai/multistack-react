...
export default function useCitiesForm(estado: string){
    ...
    citiesName = useMemo(() => (
        (enderecosAtendidos || []).map((item: CidadeInterface) => item.cidade )
    ), [enderecosAtendidos]
    ),
    options = useMemo(() => {
    ...
    function handleNewCity(newValue: string | null){
        if(newValue){
            ...
            newCity && setValue('enderecosAtendidos', [
                ...enderecosAtendidos, newCity
            ])
        }
    }

    function handleDelete(item: string){
        setValue('enderecosAtendidos', enderecosAtendidos.filter((city: CidadeInterface) => city.cidade !== item))
    }
